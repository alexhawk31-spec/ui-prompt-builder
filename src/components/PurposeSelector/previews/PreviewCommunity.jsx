    ...
    <div style={{ height: "100%", display: "flex", flexDirection: "column", padding: pad, gap: gap, background: p.bg, fontFamily: bFont }}>
      ...
      <div style={{ display: "flex", alignItems: "center", justifyContent: "space-between" }}>
        ...
        <div style={{ display: "flex", alignItems: "center", gap: 5 }}>
          <div style={{ padding: "4px 10px", borderRadius: cRad, background: p.surface, border: `1px solid ${p.border}`, fontSize: fs(8), fontWeight: 600, color: p.muted, transition: tr, ...btnS }}>Filter</div>
      ...
      {[
        ...
        <div key={pi} style={{ borderRadius: cRad, background: p.card, border: `1px solid ${p.border}`, padding: pad, boxShadow: cShadow, transition: tr, ...cBase, ...(cExtra || {}) }}>
          ...
          {post.hasImg && <div style={{ height: 42, borderRadius: cRad, background: `linear-gradient(135deg,${p.accentBg},${p.greenBg})`, border: `1px solid ${p.border}`, marginBottom: 4 }} />}
          <div style={{ display: "flex", gap: 12, marginTop: 6, paddingTop: 6, borderTop: `1px solid ${p.borderLight}` }}>
      ...
      {/* Compose card */}
      <div style={{ borderRadius: cRad, background: p.card, border: `1px solid ${p.border}`, padding: pad, display: "flex", alignItems: "center", gap, boxShadow: cShadow, transition: tr, ...cBase, ...(cExtra || {}) }}>
        ...
        <div style={{ padding: "4px 10px", borderRadius: cRad, background: p.accent, fontSize: fs(7), fontWeight: 700, color: "#fff", transition: tr, ...btnP }}>Post</div>
      </div>
    </div>
  );
}
