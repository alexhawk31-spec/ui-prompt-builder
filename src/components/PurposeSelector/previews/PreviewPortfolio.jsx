  ...
  return (
    <div style={{ height: "100%", display: "flex", flexDirection: "column", background: p.bg, fontFamily: bFont }}>
      ...
      <div style={{ display: "flex", alignItems: "center", justifyContent: "space-between", padding: `${pad}px ${pad * 1.5}px` }}>
        <div style={{ fontSize: fs(12), fontWeight: hWt, fontFamily: hFont, color: p.text }}>Jane Doe</div>
        <div style={{ display: "flex", gap: pad, alignItems: "center" }}>
          ...
          <div style={{ padding: "3px 10px", borderRadius: cRad, background: p.accent, fontSize: fs(7), fontWeight: 700, color: "#fff", transition: tr, ...btnP }}>Hire Me</div>
        </div>
      </div>
      ...
      <div style={{ flex: 1, display: "grid", gridTemplateColumns: "1fr 1fr", gap, padding: `0 ${pad * 1.5}px`, minHeight: 0 }}>
        {[
          ...
          <div key={proj.name} style={{ borderRadius: cRad, overflow: "hidden", background: p.card, border: `1px solid ${p.border}`, boxShadow: cShadow, transition: tr, ...cBase, ...(cExtra || {}) }}>
            ...
            <div style={{ padding: `${gap}px ${gap * 1.2}px` }}>
              <div style={{ fontSize: fs(10), fontWeight: hWt, fontFamily: hFont, color: p.text }}>{proj.name}</div>
      ...
      {/* Footer CTA */}
      <div style={{ display: "flex", alignItems: "center", justifyContent: "center", padding: `${pad}px`, gap, borderTop: `1px solid ${p.border}` }}>
        <div style={{ padding: `4px ${pad}px`, borderRadius: cRad, background: p.surface, border: `1px solid ${p.border}`, fontSize: fs(8), fontWeight: 600, color: p.muted, transition: tr, ...btnS }}>View Resume</div>
        <div style={{ padding: `4px ${pad}px`, borderRadius: cRad, background: p.accent, fontSize: fs(8), fontWeight: 700, color: "#fff", transition: tr, ...btnP }}>Say Hello</div>
        <div style={{ padding: `4px ${pad}px`, borderRadius: cRad, background: p.surface, border: `1px solid ${p.border}`, fontSize: fs(8), fontWeight: 600, color: p.muted, transition: tr, ...btnS }}>Download CV</div>
      </div>
    </div>
  );
}
