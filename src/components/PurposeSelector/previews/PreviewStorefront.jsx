export default function PreviewStorefront({ p, mood, cardCSS, buttonCSS, dataCSS }) {
  const m = mood || {};
  ...
  return (
    <div style={{ height: "100%", display: "flex", flexDirection: "column", background: p.bg, fontFamily: bFont }}>
      {/* Nav bar */}
      <div style={{ display: "flex", alignItems: "center", justifyContent: "space-between", padding: `${Math.round(gap)}px ${pad}px`, borderBottom: cardBorder }}>
        <div style={{ fontSize: fs(11), fontWeight: hWt, fontFamily: hFont, color: p.text, fontStyle: hIt ? "italic" : "normal" }}>Acme</div>
        ...
        <div style={{ display: "flex", gap: 4 }}>
          <div style={{ padding: "3px 8px", borderRadius: cRad, background: p.surface, border: cardBorder, fontSize: fs(7), fontWeight: 600, color: p.muted, transition: tr, ...btnS }}>Log In</div>
      ...
      <div style={{ flex: 2, background: `linear-gradient(135deg, ${p.accentBg}, ${p.gradA})`, display: "flex", flexDirection: "column", justifyContent: "center", padding: pad * 1.5, gap: gap * 0.5 }}>
        <div style={{ fontSize: fs(7), fontWeight: 600, textTransform: lblTx, letterSpacing: ".12em", color: p.accent }}>Launch Day</div>
        <div style={{ fontSize: fs(18), fontWeight: hWt, fontFamily: hFont, color: p.text, lineHeight: 1.2, fontStyle: hIt ? "italic" : "normal" }}>Ship faster,<br />build better.</div>
        <div style={{ width: "80%", height: 4, borderRadius: rad, background: `${p.dim}40`, marginTop: 2 }} />
        <div style={{ display: "flex", gap, marginTop: gap }}>
          <div style={{ padding: `${gap}px ${pad}px`, borderRadius: cRad, background: p.accent, fontSize: fs(9), fontWeight: 700, color: "#fff", transition: tr, boxShadow: accentShadow, ...btnP }}>Get Started Free</div>
          <div style={{ padding: `${gap}px ${pad}px`, borderRadius: cRad, background: "transparent", border: cardBorder, fontSize: fs(9), fontWeight: 600, color: p.muted, transition: tr, ...btnS }}>See Demo</div>
        </div>
      </div>

      {/* Social proof bar */}
      <div style={{ padding: `${gap}px ${pad}px`, borderTop: cardBorder, borderBottom: cardBorder, display: "flex", alignItems: "center", justifyContent: "center", gap: pad }}>
        {["10K+ users", "4.9 ★", "99.9% uptime"].map((t) => (
          <div key={t} style={{ fontSize: fs(8), fontWeight: 600, color: p.dim, ...(ds.number || {}) }}>{t}</div>
        ))}
      </div>
      ...
      <div style={{ flex: 1, display: "grid", gridTemplateColumns: "1fr 1fr 1fr", gap, padding: pad }}>
        {[
          ...
          <div key={f.title} style={{ borderRadius: cRad, padding: gap, background: cardBg, border: cardBorder, boxShadow: cShadow, transition: tr, ...cardBlur, ...cBase, ...(cExtra || {}) }}>
      ...
      {/* Footer CTA */}
      <div style={{ display: "flex", alignItems: "center", justifyContent: "center", padding: `${gap}px ${pad}px`, gap: 4, borderTop: cardBorder }}>
