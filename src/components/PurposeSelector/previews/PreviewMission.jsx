export default function PreviewMission({ p, mood, cardCSS, buttonCSS, dataCSS }) {
  const m = mood || {};
  ...
  return (
    <div style={{ height: "100%", display: "flex", flexDirection: "column", padding: pad, gap: gap, background: p.bg, fontFamily: bFont }}>
      <div style={{ display: "flex", alignItems: "center", justifyContent: "space-between" }}>
        <div style={{ fontSize: fs(15), fontWeight: hWt, fontFamily: hFont, color: p.text, fontStyle: hIt ? "italic" : "normal" }}>Sprint 24</div>
        <div style={{ display: "flex", alignItems: "center", gap: 4 }}>
          <div style={{ padding: "4px 10px", borderRadius: cRad, background: p.accent, fontSize: fs(8), fontWeight: 700, color: "#fff", transition: tr, boxShadow: accentShadow, ...btnP }}>Add Task</div>
      ...
      <div style={{ display: "flex", gap: 4 }}>
        {[{ l: "To Do", n: 4, bg: p.surface, c: p.muted }, { l: "In Progress", n: 3, bg: p.accentBg, c: p.accent }, { l: "Review", n: 2, bg: p.amberBg, c: p.amber }, { l: "Done", n: 7, bg: p.greenBg, c: p.green }].map((s) => (
          <div key={s.l} style={{ flex: 1, padding: "5px 8px", borderRadius: rad, background: s.bg, display: "flex", alignItems: "center", justifyContent: "space-between", transition: tr }}>
            <div style={{ fontSize: fs(7), fontWeight: 600, color: s.c, textTransform: lblTx, letterSpacing: lblSp }}>{s.l}</div>
            <div style={{ fontSize: fs(9), fontWeight: 700, color: s.c, ...(ds.number || {}) }}>{s.n}</div>
          </div>
        ))}
      </div>
      <div style={{ flex: 1, display: "flex", gap: gap, minHeight: 0 }}>
        {[
          ...
          <div key={col.title} style={{ flex: 1, borderRadius: rad, background: glass ? `${p.surface}${gAlpha}` : p.surface, border: cardBorder, padding: gap, display: "flex", flexDirection: "column", gap: 4, overflow: "hidden", boxShadow: cShadow, transition: tr, ...cardBlur }}>
            <div style={{ display: "flex", alignItems: "center", justifyContent: "space-between", padding: "0 2px" }}>
              <div style={{ fontSize: fs(7), fontWeight: 700, textTransform: lblTx, letterSpacing: lblSp, color: p.dim }}>{col.title}</div>
              <div style={{ padding: "1px 5px", borderRadius: cRad, background: p.bg, border: cardBorder, fontSize: fs(7), fontWeight: 600, color: p.dim, transition: tr, ...btnS }}>+</div>
            </div>
            {col.cards.map((card) => (
              <div key={card.t} style={{ borderRadius: cRad, padding: 8, background: card.active ? (glass ? `${p.accentBg}${gAlpha}` : p.accentBg) : (glass ? `${p.bg}${gAlpha}` : p.bg), border: `${Math.max(bdrW, 1)}px solid ${card.active ? p.accent : p.border}`, flexShrink: 0, boxShadow: card.active && glow ? `0 0 ${Math.round(glowSz * 0.5)}px ${p.accent}30` : cShadow, transition: tr, ...cardBlur, ...cBase, ...(cExtra || {}) }}>
